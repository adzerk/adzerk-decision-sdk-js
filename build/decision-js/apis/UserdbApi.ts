...
export class UserdbApi extends runtime.BaseAPI {
  ...
  async addCustomPropertiesRaw(
    ...
    if (this.configuration && this.configuration.apiKey) {
      headerParameters["X-Adzerk-ApiKey"] = this.configuration.apiKey(
        "X-Adzerk-ApiKey"
  ...
  async addInterestsRaw(
    ...
    if (
      requestParameters.interest === null ||
      requestParameters.interest === undefined
    ) {
      throw new runtime.RequiredError(
        "interest",
   ...
   */
  async forgetRaw(
    requestParameters: ForgetRequest
   ...
   */
  async optOutRaw(
    requestParameters: OptOutRequest
  ): Promise<runtime.ApiResponse<Blob>> {
    if (
      requestParameters.networkId === null ||
      requestParameters.networkId === undefined
  ...
  async optOut(networkId: number, userKey: string): Promise<Blob> {
    const response = await this.optOutRaw({
      networkId: networkId,
      userKey: userKey
    });
    return await response.value();
