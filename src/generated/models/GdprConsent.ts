/* tslint:disable */
/* eslint-disable */
/**
 * Adzerk Decision API
 * Adzerk Decision API
 *
 * The version of the OpenAPI document: 1.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import { Consent, ConsentFromJSON, ConsentFromJSONTyped, ConsentToJSON } from './';

/**
 *
 * @export
 * @interface GdprConsent
 */
export interface GdprConsent {
  /**
   *
   * @type {string}
   * @memberof GdprConsent
   */
  userKey?: string;
  /**
   *
   * @type {Consent}
   * @memberof GdprConsent
   */
  consent?: Consent;
}

export function GdprConsentFromJSON(json: any): GdprConsent {
  return GdprConsentFromJSONTyped(json, false);
}

export function GdprConsentFromJSONTyped(
  json: any,
  ignoreDiscriminator: boolean
): GdprConsent {
  if (json === undefined || json === null) {
    return json;
  }
  return {
    userKey: !exists(json, 'userKey') ? undefined : json['userKey'],
    consent: !exists(json, 'consent') ? undefined : ConsentFromJSON(json['consent']),
  };
}

export function GdprConsentToJSON(value?: GdprConsent | null): any {
  if (value === undefined) {
    return undefined;
  }
  if (value === null) {
    return null;
  }
  return {
    userKey: value.userKey,
    consent: ConsentToJSON(value.consent),
  };
}
